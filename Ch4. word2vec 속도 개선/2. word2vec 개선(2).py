...
class CBOW:
    def __init__(self,vocab_size,hidden_size,window_size,corpus):
        ...
        self.ns_loss = NegativeSamplingLoss(W_out,corpus,power=0.75,sampl_size = 5)
    ...
    def forward(self,contexts,target):
        h = 0 #넘파이는 int 0에다가 넘파이 배열을 더하는 것이 가능하다(what the...)
        for i,layer in enumerate(self.in_layers):
            h += layer.forward(contexts[:,i])  # 각 맥락 세트마다 서로 더하는 작업을 진행함, 각 맥락세트는 하나의 행 벡터로 됨.
        h *= 1 / len(self.in_layers) # 더했으니 더한 갯수만큼 나눠주기
...
from common.util import create_contexts_target, tp_cpu, to_gpu
...
word_vecs = model.word.vecs
if config.GPU:
    word_vecs = to_gpu(word_vecs)
parmas = {}
params['word_vecs'] = word_vecs.astype(np.float16)
params['word_to_id'] = word_to_id
params['word_to_id'] = id_to_word
pkl_file = 'cbow_params.pkl'
with open(pkl_file,'wb') as f:
    pickle.dump(parmas,f,-1)